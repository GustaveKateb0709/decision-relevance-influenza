...
src/
  ...
  make_figures.py             publication figures (writes into 02_Figures/ and
                              03_Supplementary/ when those directories exist,
                              otherwise into figures/)
  verify_figures.py           figure verification (reads the figure files)
...
# figures (re-runs from results/*.csv; see "Not in this repository" below for
# the verification step, which also needs the delivered figure files)
python make_figures.py
```

Everything is seeded. Reference runtime: Python 3.13.12, torch 2.13.0, numba 0.66.0,
single-threaded CPU, 8 GB unified memory. The published figures were rendered with
matplotlib 3.10.9; see `requirements.txt` for why that matters.

## Not in this repository

The figure files ship with the submission package, not here, so `verify_figures.py`
needs them dropped in before it runs. `make_figures.py` reads only committed files
and runs from a bare clone.
...
When a file is missing it prints every path it tried instead of raising a bare
`FileNotFoundError`.
